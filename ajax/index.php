...
function LoadingSalaries()
{
    ...
    if ($_SESSION['Login_Type'] == 'employee') {
        ...
        $queryConditions .= " WHERE `salary`.`emp_code` = '" . $empData['emp_code'] . "'";
    }

    if (!empty($requestData['search']['value'])) {
        $searchValue = $requestData['search']['value'];
        $queryConditions .= " AND (`salary`.`emp_code` LIKE '%$searchValue%'"
            . " OR CONCAT(TRIM(`emp`.`first_name`), ' ', TRIM(`emp`.`last_name`)) LIKE '%$searchValue%'"
            . " OR `salary`.`pay_month` LIKE '%$searchValue%'"
            . " OR `salary`.`earning_total` LIKE '%$searchValue%'"
            . " OR `salary`.`deduction_total` LIKE '%$searchValue%'"
            . " OR `salary`.`net_salary` LIKE '%$searchValue%')";
    }

    $sql = "SELECT `emp`.`emp_code`, `emp`.`first_name`, `emp`.`last_name`, `salary`.*";
    $sql .= " FROM `" . DB_PREFIX . "salaries` AS `salary`, `" . DB_PREFIX . "employees` AS `emp`";
    $sql .= " WHERE `emp`.`emp_code` = `salary`.`emp_code`";
    $sql .= $queryConditions;
...
while ($row = mysqli_fetch_assoc($query)) {
        ...
        if ($_SESSION['Login_Type'] == 'admin') {
            ...
            // Use $row['emp_code'] directly for employee URL generation
...
// Check if the arrays for earnings and deductions are empty and insert optimal values if needed
if (empty($earnings_heads) || empty($earnings_amounts)) {
    $earnings_heads = array('Default Earning');
    $earnings_amounts = array(0.00);
}

if (empty($deductions_heads) || empty($deductions_amounts)) {
    $deductions_heads = array('Default Deduction');
    $deductions_amounts = array(0.00);
}
...
if (!empty($emp_code) && !empty($pay_month)) {
    ...
    for ($i = 0; $i < count($deductions_heads); $i++) {
	    ...
	    $html .= '<img class="logo" src="../dist/img/logo.jpeg" alt="Mango Solutions EMS" />';
	    $html .= '</div>';
	    $html .= '<div class="div_half">';
	    $html .= '<h2 class="com_title">Mango Solutions EMS</h2>';
	    $html .= '<p class="reg_no">info.mangosolution@gmail.com</p>';
	    $html .= '</div>';
			...
			$html .= '<table class="emp_info">';
				$html .= '<thead>';
					$html .= '<tr>';
						...
						$html .= '<th width="200">';
							$html .= '<img width="100" src="../dist/img/signature.png" alt="Nabeel" /><br />';
							$html .= '<strong>Nabeel, CEO</strong>';
						$html .= '</th>';
					$html .= '</tr>';
				$html .= '</thead>';
			$html .= '</table>';
		...
		$html .= '<p class="com_info">';
			$html .= '45/C Badar Commercial, Street 11,<br/>';
			$html .= 'Phase V, Defence Housing Authority,<br/>';
			$html .= 'Sindh, 75500,<br/>';
			$html .= 'Karachi, Pakistan<br/>';
			$html .= 'www.mangosolution.pk';
		$html .= '</p>';
...
$payslip_path = __DIR__ . '/../payslips/';

if (!file_exists($payslip_path . $emp_code . '/')) {
    mkdir($payslip_path . $emp_code, 0777, true); // The third parameter 'true' creates parent directories if necessary.
}
if (!file_exists($payslip_path . $emp_code . '/' . $pay_month . '/')) {
    mkdir($payslip_path . $emp_code . '/' . $pay_month, 0777, true);
...
function SendPaySlipByMail() {
	...
	if ( $empData ) {
		...
		$message .= '<hr/>';
		$message .= '<p>Thank You,<br/>Mango Solutions EMS. Powered by: Gettexh.</p>';
...
function ApplyLeaveToAdminApproval() {
    ...
    $leave_subject = isset($_POST['leave_subject']) ? addslashes($_POST['leave_subject']) : '';
    $leave_dates = isset($_POST['leave_dates']) ? addslashes($_POST['leave_dates']) : '';
    $leave_message = isset($_POST['leave_message']) ? addslashes($_POST['leave_message']) : '';
    $leave_type = isset($_POST['leave_type']) ? addslashes($_POST['leave_type']) : '';
    ...
    if (!empty($leave_subject) && !empty($leave_dates) && !empty($leave_message) && !empty($leave_type)) {
        ...
        if (empty($AppliedDates)) {
            // Check for overlapping leave dates before inserting new application
            $datesToCheck = explode(',', $leave_dates);
            $overlapDates = array();

            foreach ($datesToCheck as $date) {
                $checkOverlapSQL = mysqli_query($db, "SELECT * FROM `" . DB_PREFIX . "leaves` WHERE `leave_dates` LIKE '%$date%' AND `emp_code` = '" . $empData['emp_code'] . "'");
                if ($checkOverlapSQL) {
                    if (mysqli_num_rows($checkOverlapSQL) > 0) {
                        $overlapDates[] = $date;
                    }
                }
            }
            ...
            if (empty($overlapDates)) {
                // No overlapping dates found, insert the new leave application
				...
				// Assuming you have already executed the INSERT query and stored it in $result
				...
				$empName    = $empData['first_name'] . ' ' . $empData['last_name'];
				$empEmail   = $empData['email'];
				$adminEmail = "ahmedanas37@gmail.com";
				...
				// Assuming you already have the $insertedId variable containing the inserted ID
...
$message   .= '</form>';
				...
				$send = Send_Mail($subject, $message, $empName, $adminEmail);
				// $send = Send_Mail($subject, $message, $adminName, $adminEmail, $empName, $empEmail);


					if ( $send == 0 ) {
						$result['code'] = 0;
						$result['result'] = 'Leave Application is successfully submitted. An email notification will be send to your employer.';
					} else {
						$result['code'] = 1;
						$result['result'] = 'Leave Application is not submitted, please try again.';
					}
                ...
                $alreadyDates = implode(', ', $overlapDates);
                $result['code'] = 2;
